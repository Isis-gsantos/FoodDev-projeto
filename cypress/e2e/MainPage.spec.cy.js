...
describe('FoodDev Main Page', () => {
  ...
  it('Checks the burguer, pizza and desserts buttons', ()=> {
    cy.get('#hamburguer-icon > img').click()
    cy.get('.x-tudo > img').should('be.visible')
    cy.get('.x-bacon > img').should('be.visible')
    cy.get('.x-salada > img').should('be.visible')
    cy.get('.calabresa > img').should('not.be.visible')
    cy.get('.queijo > img').should('not.be.visible')
    cy.get('.marguerita > img').should('not.be.visible')
    cy.get('.macaron > img').should('not.be.visible')
    cy.get('.brownie > img').should('not.be.visible')
    cy.get('.bolo > img').should('not.be.visible')

    cy.get('#pizza-icon > img').click()
    cy.get('.calabresa > img').should('be.visible')
    cy.get('.queijo > img').should('be.visible')
    cy.get('.marguerita > img').should('be.visible')
    cy.get('.macaron > img').should('not.be.visible')
    cy.get('.brownie > img').should('not.be.visible')
    cy.get('.bolo > img').should('not.be.visible')
    cy.get('.x-tudo > img').should('not.be.visible')
    cy.get('.x-bacon > img').should('not.be.visible')
    cy.get('.x-salada > img').should('not.be.visible')

    cy.get('#sobremesa-icon > img').click()
    cy.get('.macaron > img').should('be.visible')
    cy.get('.brownie > img').should('be.visible')
    cy.get('.bolo > img').should('be.visible')
    cy.get('.calabresa > img').should('not.be.visible')
    cy.get('.queijo > img').should('not.be.visible')
    cy.get('.marguerita > img').should('not.be.visible')
    cy.get('.x-tudo > img').should('not.be.visible')
